...
def load_PubMed(data_path, data_name, is_normalize):
    ...
    return new_g, features, torch.LongTensor([pos_link_s, pos_link_t]), torch.LongTensor([test_link_s, test_link_t]), torch.LongTensor(flags)
...
def load_DBLP(data_path, data_name, is_normalize):
    ...
    test_link_t = []
    flags = []
    with open(path + link_test_file, encoding='utf-8') as f:
        line_data = f.readline()
        while (line_data):
            s_id, t_id, flag = line_data.split()
...
def neg_sample(src, min, max):
    neg_src = src
    neg_dst = torch.randint(min, max, (src.shape[0],))
