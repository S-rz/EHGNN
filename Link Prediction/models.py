...
class EHGNN_yelp(torch.nn.Module):
    def __init__(self, in_feat, hidden, out_feat, n_layer, alpha, n_metapath, n_types, wo_l2, wo_mweight, wo_tweight, dropout):
        super(EHGNN_yelp, self).__init__()
        self.out_feat = out_feat
        self.alpha = alpha
        self.metapath_weights = torch.nn.Parameter((torch.zeros(n_metapath) + 1.))
        self.ntype_weights = torch.nn.Parameter((torch.zeros(n_types) + 1.))
    ...
    def forward(self, X, s_features, s_idxs, t_idxs, weightss, t_typess, weight_type, batch_size, device):
        ...
        if self.wo_mweight:
            nm_weigths = torch.ones(nm_per)
        else:
            nm_weigths = torch.softmax(self.metapath_weights[weight_type * nm_per : (weight_type + 1) * nm_per], dim=0)
        if self.wo_tweight:
            nt_weights = torch.ones(nt_per)
        else:
            nt_weights = torch.softmax(self.ntype_weights[weight_type * nt_per : (weight_type + 1) * nt_per], dim=0)
